...
int main(int argc, char *argv[]) {
    ...
    if(width <= 0 || height <= 0){
        if(rank == 0){
            printf("Mr.Genius, how can you create such a map!\n");
        }
        return -1;
    }
    ...
    if (rank == 0) {
        ...
        /*
         * Initialize map
         * This part can be parallelized. The corresponding explanation is presented in the Prospect
         * section in the coursework report
         */
        ...
        printf("percolate: rho = %f, actual density = %f\n", rho, 1.0 - ((double) nhole) / ((double) width * height));
    }

    // Broadcast map to all processes
    MPI_Bcast(&map[0][0], width * height, MPI_INT, 0, comm);
    ...
    while (step <= maxstep) {
        // Exchange boundaries
        MPI_Isend(&old[1][1], 1, row_boundary, left, 1, new_comm, &request[0]);
        MPI_Isend(&old[1][col_elements], 1, row_boundary, right, 2, new_comm, &request[1]);
        MPI_Isend(&old[1][1], 1, col_boundary, top, 3, new_comm, &request[2]);
        MPI_Isend(&old[row_elements][1], 1, col_boundary, down, 4, new_comm, &request[3]);

        nchangelocal = 0;
        // Update non-boundary elements
        for (i = 2; i < row_elements; i++) {
            for (j = 2; j < col_elements; j++) {
                oldval = old[i][j];
                newval = oldval;
        ...
        // Update boundary maps and check if there is any change
        for (i = 1; i < row_elements+1; i += row_elements-1) {
            for (j = 1; j < col_elements+1; j++) {
                oldval = old[i][j];
                newval = oldval;
        ...
        for (i = 1; i < row_elements+1; i++) {
            for (j = 1; j < col_elements+1; j+=col_elements-1) {
                oldval = old[i][j];
                newval = oldval;
    ...
    MPI_Reduce(&is_percolate, &is_percolate, 1, MPI_C_BOOL, MPI_LOR, 0, comm);
